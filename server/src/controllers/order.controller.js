...
async function buildOrderFromCart(items) {
  ...
  for (const it of items) {
    ...
    orderItems.push({
      ...
      lineTotalPaise,
      farmerId: crop.farmerId, // save farmer reference
    });
...
exports.createOrder = async (req, res, next) => {
  ...
  try {
    ...
    const order = await Order.create(
      {
        buyerId: req.user.id,
        farmerId: orderItems[0].farmerId, // link to the first farmer (extend later for multi-farmer support)
        amount: totalPaise,
        currency: "INR",
        status: "created",
  ...
  try {
    ...
    if (isValid) {
      ...
      for (const it of order.orderItems) {
        ...
        const newQty =
          parseFloat(crop.quantityKg) - parseFloat(it.quantityKg);
